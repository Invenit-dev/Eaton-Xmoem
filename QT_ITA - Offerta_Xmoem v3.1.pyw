"""
V3.1 : aggiunta ricerca automatica del file 00_Listino_MM_AAAA_XMOEM
...
file_name = next((f for f in os.listdir(current_dir) if f.startswith("00_Listino") and f.endswith(".xlsx")), None)
listino_mese = file_name[11:13]
listino_anno = file_name[14:18]

# Carica i dati da Excel
listino_df = pd.read_excel(os.path.join(current_dir, file_name), sheet_name=0, engine='openpyxl', header=None)
sconti_df = pd.read_excel(os.path.join(current_dir, file_name), sheet_name=1, engine='openpyxl', header=None)
rubrica_df = pd.read_excel(os.path.join(current_dir, file_name), sheet_name=2, engine='openpyxl', header=None)
# Crea dizionari
...
if not (
        os.path.basename(f).startswith("00_Listino") or
