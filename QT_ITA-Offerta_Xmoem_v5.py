"""
V5 : Aggiunta ricerca automatica del file 00_Listino_MM_AAAA_XMOEM
    Compilato di conseguenza: cella B51 del Riassuntivo e D20 del Preventivo
    Aggiunto grassetto sulle intestazioni di entrambe le pagine
    Aggiunte formule per copiare Riferimento e Descrizione Quadro da Riassuntivo a Preventivo
    Aggiunta Area di stampa A4
    Aggiunta lettura data listino ed in caso il mese sia 99, viene creato un Output come le vecchie versioni per copiare le righe nel excel Xmoem originale
    Aggiunta esecuzione da .exe
    Aggiunta identificazione Nome Quadro e Descrizione Quadro a partire dal nome file excel, oltre che rimozione enumerazione iniziale
"""
...
def formattazione_iniziale(riga):
    ...
    riassuntivoSheet.write(8,col_right_num, "", highlight_grn_lft_bold)
...
for file_path in excel_files:
    ...
    if match:
        ...
        print("Formato nome file non riconosciuto.")
    ...
    for idx, code in enumerate(input_df.iloc[:, 0]):
        # Salta righe vuote o che contengono intestazioni comuni (come "CODICE", "QTY", ecc.).
        ...
        if row is not None:
            # Controlla se la cella in colonna 8 (row.iloc[7]) contiene una stringa specifica
            evidenzia = (
                isinstance(row.iloc[7], str) and
                "non compatibile con il nuovo sistema ProfiSNAP" in row.iloc[7]
            )
            if evidenzia: 
        ...
        else: #codice articolo non trovato nel listino
            row_data = ["", code_str, "", f"=J{excel_row}", qty, f"=D{excel_row}*E{excel_row}", "", "", "",
                        "XXX",
                        f"=VLOOKUP(I{excel_row}, 'Famiglie Sconto'!A:E, 3, FALSE)",  # K
                        f"=VLOOKUP(I{excel_row}, 'Famiglie Sconto'!A:E, 4, FALSE)",  # L
                        f"=VLOOKUP(I{excel_row}, 'Famiglie Sconto'!A:E, 5, FALSE)",  # M
