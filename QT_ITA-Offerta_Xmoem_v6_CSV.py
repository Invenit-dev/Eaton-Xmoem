...
#print(f"Numero righe nel dizionario: {len(listino_dict)}")
...
for file_path in excel_files:
    ...
    if match:
        ...
        print("Formato nome file non standard.")
    ...
    for idx, row_input in enumerate(input_df.itertuples(index=False, name=None)):
        ...
        if row is not None:
            ...
            row_data = [
                ...
                row[3],  # H
                row[6],  # I
                str(row[4]).replace('.', ',') if not pd.isna(row[4]) else "N/A",  # J  Prezzo Listino
